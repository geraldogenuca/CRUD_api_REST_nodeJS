...
1 - Install Insomnia Download: https://insomnia.rest/download
  ...
  <img src="https://github.com/geraldogenuca/CRUD_api_REST_nodeJS/blob/main/public/imgsReadme/insomnia04.png">
...
2 - Register API
    In order not to make a long tutorial, I will be very basic because the titles are very intuitive, so I will talk about a table and the others follow the same registration scheme.

  - Registering API customers.
    We will start with this record because, only general search and search for a specific item and allowed to all, plus the fields of registration, update and delete, can only be done by logged customers, follows the intrudes.

    In image the abix shows the fields that the registration calls and next to it will show the result, if positive show the inserted data, if there is error appear the error regarding what is missing or is wrong.

    <img src="https://github.com/geraldogenuca/CRUD_api_REST_nodeJS/blob/main/public/imgsReadme/registerCostumer.jpg">

    Then we will go to the login, it would ask for the customer's email and password, after the action beside the action will appear the authentication token, and if there is a fault will show the respective error.
    ...
    Go to the login copy the token generated. Then click update, va in the auth tab, which is below this field line, there will have a arrow click on it, then click where the "Bearer Token" field has, there in the Token field copy the token generated in the login. Ready now that you can do the update, repeat this to all registration, change and exclusion fields.    
